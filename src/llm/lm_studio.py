...
def require_model_loaded(api_base: str | None, model: str) -> ModelRecord:
    """Prove the exact model is resident before inference, per /api/v0/models state.

    The native registry is the only trustworthy source of loaded state: LM Studio
    lists every downloaded model on /v1/models regardless of the JIT setting.
    Requests are only ever sent to a model verified resident here, so no request
    from this client can trigger a server-side autoload.
    """
    registry = LMStudioRegistry.fetch_quietly(api_base)
    return registry.require_loaded(model)
