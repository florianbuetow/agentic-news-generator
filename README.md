...
│       ├── content/       # Articles (Markdown) + layout.json (@nuxt/content)
│       ├── pages/
...
└── data/
    ...
    ├── input/
    │   └── newspaper/
    │       └── articles/    # Source article Markdown (preprocessed into the frontend)
...
```
frontend/newspaper/
├── nuxt.config.ts          # Nuxt config with static generation (dev port 12000)
...
│   └── layout.json         # Front-page layout: hero/featured/secondary/sidebar/briefs by slug
├── layouts/
│   └── default.vue         # Default page layout
└── pages/
    ├── index.vue           # Homepage (queries the articles + layout collections)
...
Content is managed through `@nuxt/content` (see `frontend/newspaper/content.config.ts`), which defines two collections:
...
- **`layout`** — `content/layout.json`, which arranges the front page by article slug:
  ...
  - `briefs[][]` - News briefs organized into columns

The homepage (`pages/index.vue`) reads both collections via `queryCollection()` and maps articles into sections according to `layout.json`.
...
- **Content-driven** - articles authored as Markdown, arranged by `content/layout.json` via `@nuxt/content`
...
This will:
1. Validate and preprocess the article markdown into `frontend/newspaper/content/articles/`
2. Install npm dependencies (if needed)
3. Start the development server at `http://localhost:12000`
