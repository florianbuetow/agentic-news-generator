"""Unit tests for AttributionAgent and MockAttributionAgent."""
...
def make_agent(*, tmp_path: Path, llm: RecordingLLMClient, loader: RecordingPromptLoader) -> AttributionAgent:
...
class TestAttributionAgent:
    ...
    def test_rewrite_verdict_parsed(self, tmp_path: Path) -> None:
        ...
        assert "attribution" in result.output.suggested_fix.lower()
    ...
    def test_keep_verdict_parsed(self, tmp_path: Path) -> None:
        ...
        assert result.output.status == "REWRITE"
        assert result.output.suggested_fix is not None
        assert "suggested" in result.output.suggested_fix

    def test_keep_null_fix(self, tmp_path: Path) -> None:
    ...
    def test_mock_implements_protocol(self) -> None:
        result = MockAttributionAgent().evaluate(
            concern=make_test_concern(3),
            article=make_test_article(),
            source_text="Source text",
            source_metadata=make_test_metadata(),
            style_requirements="SCIAM_MAGAZINE",
    ...
    def test_mock_returns_keep_with_matching_id(self) -> None:
        ...
        assert result.output.concern_id == 3
        assert result.output.status == "KEEP"
