"""Unit tests for ArticleReviewAgent and MockArticleReviewAgent."""
...
def make_agent(*, tmp_path: Path, llm: RecordingLLMClient, loader: RecordingPromptLoader) -> ArticleReviewAgent:
    config_path = _write_test_config(tmp_path)
    return ArticleReviewAgent(
        llm_config=make_test_llm_config(),
        config=Config(config_path),
        llm_client=llm,
        prompt_loader=cast(Any, loader),
        prompt_file="article_review.md",
    )


class TestArticleReviewAgent:
    def test_review_returns_raw_bullets(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient(["- Concern A"])
        ...
        assert isinstance(result.output, ArticleReviewRaw)
        assert result.output.markdown_bullets == "- Concern A"

    def test_response_preserved_as_is(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient(["- A\n- B"])
        loader = RecordingPromptLoader()
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
        ...
        assert result.output.markdown_bullets == "- A\n- B"

    def test_empty_response_signals_no_concerns(self, tmp_path: Path) -> None:
        ...
        assert result.output.markdown_bullets == ""
    ...
    def test_prompt_loaded_via_loader(self, tmp_path: Path) -> None:
        ...
        assert loader.load_calls == ["article_review.md"]

    def test_template_formatted_with_variables(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient(["- A"])
        loader = RecordingPromptLoader(template="{source_text} {source_metadata} {generated_article}")
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
        ...
        assert json.dumps(source_metadata, ensure_ascii=False) in prompt
        assert json.dumps(article.model_dump(), ensure_ascii=False) in prompt

    def test_response_stripped(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient(["  \n- A\n  "])
        loader = RecordingPromptLoader()
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
    ...
    def test_prompt_references_article_and_source(self) -> None:
        prompt_path = Path(__file__).resolve().parent.parent / "prompts" / "article_editor" / "article_review.md"
        content = prompt_path.read_text(encoding="utf-8").lower()

        assert "article" in content
        assert "source" in content

    def test_no_json_parsing_attempted(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient(['{"key":"val"}'])
        loader = RecordingPromptLoader()
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
        ...
        assert result.output.markdown_bullets == '{"key":"val"}'
    ...
    def test_mock_returns_empty(self) -> None:
        result = MockArticleReviewAgent().review(
            article=make_test_article(),
            source_text="Source",
            source_metadata=make_test_metadata(),
        )
