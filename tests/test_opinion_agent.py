"""Unit tests for OpinionAgent and MockOpinionAgent."""
...
def make_agent(*, tmp_path: Path, llm: RecordingLLMClient, loader: RecordingPromptLoader) -> OpinionAgent:
...
class TestOpinionAgent:
    ...
    def test_prompt_loaded_via_specialist_loader(self, tmp_path: Path) -> None:
        ...
        assert loader.load_specialist_calls == [("specialists", "opinion.md")]
    ...
    def test_rewrite_verdict_parsed(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient(
            [make_verdict_json(misleading=True, status="REWRITE", suggested_fix="Add qualifier: 'Altman suggested...'")]
        )
        loader = RecordingPromptLoader()
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
    ...
    def test_keep_verdict_parsed(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient([make_verdict_json(status="KEEP", suggested_fix=None)])
        loader = RecordingPromptLoader()
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
    ...
    def test_remove_verdict_parsed(self, tmp_path: Path) -> None:
        llm = RecordingLLMClient([make_verdict_json(misleading=True, status="REMOVE", suggested_fix="Remove claim")])
        loader = RecordingPromptLoader()
        agent = make_agent(tmp_path=tmp_path, llm=llm, loader=loader)
    ...
    def test_mock_implements_protocol(self) -> None:
        result = MockOpinionAgent().evaluate(
            concern=make_test_concern(5),
            article=make_test_article(),
            source_text="Source",
            source_metadata=make_test_metadata(),
            style_requirements="SCIAM_MAGAZINE",
    ...
    def test_mock_returns_keep_with_matching_id(self) -> None:
        ...
        assert result.output.concern_id == 5
        assert result.output.status == "KEEP"
